...
extern "C" int jniRegisterNativeMethods(C_JNIEnv* env, const char* className,
	...
	LOGV(TAG, "Registering %s natives", className);

	scoped_local_ref<jclass> c(env, findClass(env, className));
	if (c.get() == NULL) {
		LOGE(TAG, "Native registration unable to find class '%s', aborting",
				className);
		abort();
	}

	if ((*env)->RegisterNatives(e, c.get(), gMethods, numMethods) < 0) {
		LOGE(TAG, "RegisterNatives failed for '%s', aborting", className);
		abort();
	}
...
extern "C" int jniThrowException(C_JNIEnv* env, const char* className,
	...
	if ((*env)->ExceptionCheck(e)) {
		...
		if (exception.get() != NULL) {
			...
			LOGE(TAG, "Discarding pending exception (%s) to throw %s", text,
					className);
			free(text);
		}
	}
...
static void tryInitCachedFields(JNIEnv* env) {
	...
	if (gCachedFields.fileDescriptorClass == NULL) {
		LOGE(TAG, "tryInitCachedFields(fileDescriptorClass) failed");
		abort();
	}
	...
	if (gCachedFields.fileDescriptorCtor == NULL) {
		LOGE(TAG, "tryInitCachedFields(fileDescriptorCtor) failed");
		abort();
	}
	...
	if (gCachedFields.descriptorField == NULL) {
		LOGE(TAG, "tryInitCachedFields(descriptorField) failed");
		abort();
	}
	return;
}
...
void jniSetFileDescriptorOfFD(C_JNIEnv* env, jobject fileDescriptor,
	...
	JNIEnv* e = reinterpret_cast<JNIEnv*>(env);
	tryInitCachedFields(e);
	(*env)->SetIntField(e, fileDescriptor, gCachedFields.descriptorField,
			value);
}
...
extern "C" jbyte* jniGetNonMovableArrayElements(C_JNIEnv* env,
	...
	jbyte* result = (*env)->GetByteArrayElements(e, byteArray,
			reinterpret_cast<jboolean*>(&noCopy));
